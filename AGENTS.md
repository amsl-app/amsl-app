...
# Regenerate the provider dependency graph (-> docs/provider_graph.md)
python3 scripts/provider_graph.py
```
...
To inspect how providers depend on each other, run `python3 scripts/provider_graph.py` — it parses the declared `dependencies: [...]` across `lib/` and writes a Mermaid graph to `docs/provider_graph.md` (roots = providers with no deps; `HikariPod` is the most depended-on hub).
